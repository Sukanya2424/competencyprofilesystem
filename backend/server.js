...
app.get('/certification/:id', (req, res) => {
  ...
  const sql = `
  SELECT
  ce.id_certification, ce.code_certification, ce.university
      FROM career_level as cl
      LEFT JOIN career AS c ON c.id_career = cl.id_career
      LEFT JOIN level AS l ON l.id_level = cl.id_level
      LEFT JOIN all_details AS ad ON ad.id_career_level = cl.id_career_level
      LEFT JOIN details AS d ON d.id_d = ad.id_d
      LEFT JOIN d_certification AS dc ON dc.id_d = ad.id_d
      LEFT JOIN certification AS ce ON ce.id_certification = dc.id_certification
    WHERE cl.id_career_level = ?
  `;
  ...
  db.query(sql, [id], (error, result) => {
    if (error) {
      ...
      // Initialize arrays to store unit code and name
      const idCertificationArray = [];
      const CodecertificationArray = [];
      const universityArray = [];
      
      // Iterate through the query result to collect unit code and name
      for (const row of result) {
        idCertificationArray.push(row.id_certification);
        CodecertificationArray.push(row.code_certification);
        universityArray.push(row.university);
      }
      ...
      if (result.length === 0) {
        ...
        res.status(200).json({
          ...
          data: {
            id_career_level: result[0].id_career_level,
            id_certification: idCertificationArray,
            code_certification: CodecertificationArray,
...
app.get('/TargetGroup/:id', (req, res) => {
  const id = req.params.id; // Get the career level ID from the URL parameter
  const sql = `
  SELECT 
  t.id_target_group, t.target_group
      FROM career_level as cl
      LEFT JOIN career AS c ON c.id_career = cl.id_career
      LEFT JOIN level AS l ON l.id_level = cl.id_level
      LEFT JOIN all_details AS ad ON ad.id_career_level = cl.id_career_level
      LEFT JOIN details AS d ON d.id_d = ad.id_d
      LEFT JOIN d_target_group AS dt ON dt.id_d = ad.id_d
      LEFT JOIN target_group AS t ON t.id_target_group = dt.id_target_group
    WHERE cl.id_career_level = ?
  `;
  ...
  db.query(sql, [id], (error, result) => {
    if (error) {
      ...
      // Initialize arrays to store unit code and name
      const idTargetGroupArray = [];
      const TargetGroupArray = [];
      const descriptionArray = [];
      
      // Iterate through the query result to collect unit code and name
      for (const row of result) {
        idTargetGroupArray.push(row.id_target_group);
        TargetGroupArray.push(row.target_group);
        descriptionArray.push(row.description);
      }
      ...
      if (result.length === 0) {
        ...
        res.status(200).json({
          ...
          data: {
            id_career_level: result[0].id_career_level,
            id_target_group: idTargetGroupArray,
            target_group: TargetGroupArray,
...
app.post('/createCertification', (req, res) => {
  const { code_certification, university } = req.body;

  const newCareer = { code_certification, university };

  db.query('INSERT INTO certification SET ?', newCareer, (err, results) => {
    if (err) {
      return res.status(500).json({ error: 'Create operation failed' });
    }
...
app.get('/getCertification', (req, res) => {
  const sql = `
  SELECT * 
  FROM certification
  `;
...
// Read - Retrieve a record by ID
app.get('/getCertification/:id', (req, res) => {
  const certificationId = req.params.id;

  db.query('SELECT * FROM certification WHERE id_certification = ?', [certificationId], (err, results) => {
    if (err) {
      return res.status(500).json({ error: 'Read operation failed' });
    }
...
// Update - Modify an existing record
app.put('/updateCertification/:id', (req, res) => {
  const certificationId = req.params.id;
  const { code_certification, university } = req.body;

  const updatedCertification = { code_certification, university };

  db.query('UPDATE certification SET ? WHERE id_certification = ?', [updatedCertification, certificationId], (err, results) => {
    if (err) {
      return res.status(500).json({ error: 'Update operation failed' });
    }
...
// Delete - Remove a record by ID
app.delete('/deleteCertification/:id', (req, res) => {
  const certificationId = req.params.id;

  db.query('DELETE FROM certification WHERE id_certification = ?', [certificationId], (err, results) => {
    if (err) {
      return res.status(500).json({ error: 'Delete operation failed' });
    }
...
//Details
// Create - Add a new record
app.post('/createDetailsCertification', (req, res) => {
  const { id_d, id_certification } = req.body;

  const newDetailsCertification = { id_d, id_certification };

  db.query('INSERT INTO d_certification SET ?', newDetailsCertification, (err, results) => {
    if (err) {
      return res.status(500).json({ error: 'Create operation failed' });
    }
...
app.get('/getDetailsCertification', (req, res) => {
  const sql = `
  SELECT * 
  FROM d_certification AS dc
  LEFT JOIN details AS d ON d.id_d = dc.id_d
  LEFT JOIN certification AS c ON c.id_certification = dc.id_certification
  `;
...
// Read - Retrieve a record by ID
app.get('/getDetailsCertification/:id', (req, res) => {
  const detailsCertificationId = req.params.id;

  db.query('SELECT * FROM d_certification WHERE id_d_certification = ?', [detailsCertificationId], (err, results) => {
    if (err) {
      return res.status(500).json({ error: 'Read operation failed' });
    }
    ...
    const detailsCertification = results[0];
    return res.json(detailsCertification);
  });
});

// Update - Modify an existing record
app.put('/updateDetailsCertification/:id', (req, res) => {
  const detailCertificationId = req.params.id;
  const { id_d, id_certification } = req.body;

  const updateddetailCertification = { id_d, id_certification };

  db.query('UPDATE d_certification SET ? WHERE id_d_certification = ?', [updateddetailCertification, detailCertificationId], (err, results) => {
    if (err) {
      return res.status(500).json({ error: 'Update operation failed' });
    }
...
// Delete - Remove a record by ID
app.delete('/deleteDetailsCertification/:id', (req, res) => {
  const detailCertificationId = req.params.id;

  db.query('DELETE FROM d_certification WHERE id_d_certification = ?', [detailCertificationId], (err, results) => {
    if (err) {
      return res.status(500).json({ error: 'Delete operation failed' });
    }
...
app.post('/createDetailsTargetGroup', (req, res) => {
  const { id_d, id_target_group } = req.body;

  const newDetailsTargetGroup = { id_d, id_target_group };

  db.query('INSERT INTO d_target_group SET ?', newDetailsTargetGroup, (err, results) => {
    if (err) {
      return res.status(500).json({ error: 'Create operation failed' });
    }
...
app.get('/getDetailsTargetGroup', (req, res) => {
  const sql = `
  SELECT * 
  FROM d_target_group AS dt
  LEFT JOIN details AS d ON d.id_d = dt.id_d
  LEFT JOIN target_group AS t ON t.id_target_group = dt.id_target_group
  `;
...
// Read - Retrieve a record by ID
app.get('/getDetailsTargetGroup/:id', (req, res) => {
  const detailsTargetGroupId = req.params.id;

  db.query('SELECT * FROM d_target_group WHERE id_d_target_group = ?', [detailsTargetGroupId], (err, results) => {
    if (err) {
      return res.status(500).json({ error: 'Read operation failed' });
    }
    ...
    const detailsTargetGroup = results[0];
    return res.json(detailsTargetGroup);
  });
});

// Update - Modify an existing record
app.put('/updateDetailsTargetGroup/:id', (req, res) => {
  const detailTargetGroupId = req.params.id;
  const { id_d, id_target_group } = req.body;

  const updateddetailTargetGroup = { id_d, id_target_group };

  db.query('UPDATE d_target_group SET ? WHERE id_d_target_group = ?', [updateddetailTargetGroup, detailTargetGroupId], (err, results) => {
    if (err) {
      return res.status(500).json({ error: 'Update operation failed' });
    }
...
// Delete - Remove a record by ID
app.delete('/deleteDetailsTargetGroup/:id', (req, res) => {
  const detailTargetGroupId = req.params.id;

  db.query('DELETE FROM d_target_group WHERE id_d_target_group = ?', [detailTargetGroupId], (err, results) => {
    if (err) {
      return res.status(500).json({ error: 'Delete operation failed' });
    }
...
// Create - Add a new record
app.post('/createTargetGroup', (req, res) => {
  const { target_group  } = req.body;

  const newTargetGroup = { target_group  };

  db.query('INSERT INTO target_group SET ?', newTargetGroup, (err, results) => {
    if (err) {
      return res.status(500).json({ error: 'Create operation failed' });
    }
...
app.get('/getTargetGroup', (req, res) => {
  const sql = `
  SELECT * 
  FROM target_group
  `;
...
// Read - Retrieve a record by ID
app.get('/getTargetGroup/:id', (req, res) => {
  const TargetGroupId = req.params.id;

  db.query('SELECT * FROM target_group WHERE id_target_group = ?', [TargetGroupId], (err, results) => {
    if (err) {
      return res.status(500).json({ error: 'Read operation failed' });
    }
    ...
    const targetgroup = results[0];
    return res.json(targetgroup);
  });
});

// Update - Modify an existing record
app.put('/updateTargetGroup/:id', (req, res) => {
  const TargetGroupId = req.params.id;
  const { target_group } = req.body;

  const updatedTargetGroup = { target_group };

  db.query('UPDATE target_group SET ? WHERE id_target_group = ?', [updatedTargetGroup, TargetGroupId], (err, results) => {
    if (err) {
      return res.status(500).json({ error: 'Update operation failed' });
    }
...
// Delete - Remove a record by ID
app.delete('/deleteTargetGroup/:id', (req, res) => {
  const TargetGroupId = req.params.id;

  db.query('DELETE FROM target_group WHERE id_target_group = ?', [TargetGroupId], (err, results) => {
    if (err) {
      return res.status(500).json({ error: 'Delete operation failed' });
    }
